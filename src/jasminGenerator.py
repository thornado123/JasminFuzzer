        ...
        if action == JN.Pexpr:
            ...
            if action == "array":
                ...
                if var is not None:
                    ...
                    return var + "[" + self.expressions(action=JN.Pexpr, scope=JT.INT, r_depth=r_depth) + "]"
    ...
    def instructions(self, action=None, r_depth=0, scope=None):
        ...
        if action == JN.Pinstr:
            ...
            if action == "assign":
                ...
                return var_to_assign + assign_op + value_to_assign + ";"

            if action == "if":

                return "if " + self.expressions(action=JN.Pexpr, scope=JT.BOOL, evaluation_type=JT.BOOL, r_depth=r_depth) \
                       + self.instructions(action=JN.Pblock, r_depth=r_depth, scope=JS.Variables)

            if action == "ifelse":

                return "if " + self.expressions(action=JN.Pexpr, scope=JT.BOOL, evaluation_type=JT.BOOL)\
                       + self.instructions(action=JN.Pblock, r_depth=r_depth, scope=JS.Variables) \
                       + " else " + self.instructions(action=JN.Pblock, r_depth=r_depth, scope=JS.Variables)
        ...
        if action == JN.Pblock:
            ...
            return "{\n" + self.instructions(action=JN.Pinstr, scope=JS.Variables, r_depth=r_depth) + "\n}"
